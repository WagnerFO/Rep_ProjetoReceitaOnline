...
>>>>>>> bbd7f8fd95bbf3765526c52bf3d6ab634227426c
            ...
            case 1:
                ...
                System.out.println("Essa Receita possui açúcar? (Sim/Não):");
                String resposta = scanner.next().trim().toLowerCase();
                boolean contemAcucar = resposta.equals("sim");
                scanner.nextLine(); // Consumir a quebra de linha após o `next()`
                ...
                // Pedir o nome da categoria
                System.out.println("Digite o nome da categoria:");
                String nomeCategoria = scanner.nextLine();
                Categoria categoria = new Categoria();
                categoria.setNome(nomeCategoria); // Usar o nome em vez do ID
            ...
            case 2:
                ...
                // Buscar a receita, o método buscar agora não retorna valor, então temos que verificar a receita
                repositorioReceitaSobremesaSql.buscar(receitaS2); // Preenche a receita com as informações
                
                // Verificar se a receita foi encontrada
                if (receitaS2.getId() != 0) {
                    // Imprimir informações da receita
                    System.out.println("Receita Encontrada:");
                    System.out.println("Título: " + receitaS2.getTitulo());
                    System.out.println("Descrição: " + receitaS2.getDescricao());
                    System.out.println("Modo de Preparo: " + receitaS2.getModoPreparo());
                    System.out.println("Contem Acucar: " + receitaS2.isContemAcucar());
                    System.out.println("Tipo de Acucar: "+ receitaS2.getTipoAcucar());
                    
                    // Imprimir a categoria
                    System.out.println("Categoria: " + receitaS2.getCategoria().getNome());

                    // Imprimir os ingredientes
                    System.out.println("\nIngredientes:");
                    for (Ingrediente ingrediente : receitaS2.getIngredientes()) {
                        System.out.println(ingrediente.getNome() + ": " + ingrediente.getQuantidade() + " unidades");
                    }
                } else {
                    System.out.println("Receita não encontrada!");
                ...
                case 3:
	                ...
	                System.out.println("Digite o ID da categoria:");
	                int idCategoria3 = scanner.nextInt();
	                scanner.nextLine(); // Consome a quebra de linha após o nextInt()
	
	                // Agora buscamos a categoria no banco de dados usando o ID fornecido
	                Categoria categoria3 = repositorioCategoriaSql.buscar(idCategoria3); // Método buscar no repositório de categorias
	                if (categoria3 == null) {
	                    System.out.println("Categoria com ID " + idCategoria3 + " não encontrada.");
	                    break;  // Encerra a operação caso a categoria não seja encontrada
	                }
	
	                System.out.println("Essa Receita possui açúcar? (Sim/Não):");
	                String resposta3 = scanner.next().trim().toLowerCase();
	                boolean contemAcucar3 = resposta3.equals("sim");
	                scanner.nextLine(); // Consumir a quebra de linha após o `next()`
	                ...
	                List<Ingrediente> ingredientes3 = new ArrayList<>();
	                for (String ingredienteInput : ingredientesInput3.split(",")) {
	                    String[] partes3 = ingredienteInput.split(":");
	                    if (partes3.length == 2) {
	                        String nomeIngrediente3 = partes3[0].trim();
	                        try {
	                            double quantidadeIngrediente = Double.parseDouble(partes3[1].trim());
	                            if (quantidadeIngrediente <= 0) {
	                                System.out.println("A quantidade do ingrediente deve ser um valor positivo: " + nomeIngrediente3);
	                                continue;
	                            }
	                            Ingrediente ingrediente3 = new Ingrediente(nomeIngrediente3, quantidadeIngrediente);
	                            ingredientes3.add(ingrediente3);
	                        } catch (NumberFormatException e) {
	                            System.out.println("Erro ao interpretar a quantidade para o ingrediente: " + nomeIngrediente3);
	                        }
	                    } else {
	                        System.out.println("Formato inválido para o ingrediente: " + ingredienteInput);
	                    }
